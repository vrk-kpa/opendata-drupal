...
if [[ "${DEV_MODE}" == "true" ]]; then
  ...
  cp -aur ${MOD_DIR}/opendata-assets/resources/. ${WWW_DIR}/resources
  ...
  if [[ "$(cat ${SITE_DIR}/.fs-done)" != "$DRUPAL_IMAGE_TAG" ]]; then
    ...
    cp -aur ${BASE_DIR}/sites/. ${SITE_DIR} &
    cp -aur ${BASE_DIR}/themes/. ${THEME_DIR} &
    cp -aur ${BASE_DIR}/modules/. ${MOD_DIR} &
    cp -aur ${BASE_DIR}/core/. ${CORE_DIR} &
    cp -aur ${BASE_DIR}/resources/. ${WWW_DIR}/resources &
    wait
